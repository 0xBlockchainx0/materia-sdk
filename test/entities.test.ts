...
import { ChainId, IETH as _IETH, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const IETH = _IETH[ChainId.RINKEBY]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(IETH, decimalize(1234, IETH.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([IETH]))
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(IETH, decimalize(10, IETH.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(IETH, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(IETH, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(IETH, '1234567000000000000000')
